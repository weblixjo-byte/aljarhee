...
function ProductDetailContent() {
  ...
  if (!product) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4 bg-[#f8fafc] text-center px-4">
        <h2 className="text-xl font-black text-slate-800">عذراً، لم نجد هذا المنتج</h2>
        <p className="text-slate-400 text-xs font-bold max-w-xs">
          يبدو أن المنتج الذي تبحث عنه غير متوفر حالياً أو تم حذفه من قاعدة البيانات.
        </p>
        <Link
          href="/store"
          className="flex items-center gap-2 bg-[#2d7a1f] hover:bg-[#246118] text-white px-6 py-2.5 rounded-xl font-black text-xs transition-all cursor-pointer"
        >
          <ArrowRight size={14} />
            ...
            <div className="flex items-center justify-between border-t border-slate-50 pt-5 mt-2">
              ...
              <button
                onClick={() => {
                  ...
                  showToast(
                    !isFavorite ? "تم إضافة القطعة للمفضلة" : "تم إزالة القطعة من المفضلة",
                    "success"
                  );
                }}
