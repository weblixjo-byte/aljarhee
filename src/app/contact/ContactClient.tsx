...
export default function ContactClient() {
  ...
  // Submit handler: Sends inquiry via Web3Forms email only
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!web3formsKey) {
      showToast("خدمة الإيميل غير مفعّلة حالياً، يرجى التواصل عبر الأرقام المباشرة.", "error");
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        showToast("تم إرسال استفسارك بنجاح! سيتواصل معك فريق المبيعات قريباً.", "success");
        setName("");
        setPhone("");
        setCarDetails("");
        setChassisNumber("");
      ...
      showToast("حدث خطأ أثناء الإرسال، يرجى المحاولة مرة أخرى أو التواصل عبر الهاتف.", "error");
    } finally {
  ...
  return (
    <main className="flex-1">
      ...
      <section className="bg-slate-50/70 border-b border-slate-200/60 pt-28 pb-14 text-right">
        <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-2 text-xs font-bold text-slate-400 mb-4 justify-start">
            ...
            <span className="text-slate-700">تواصل معنا</span>
          </div>
          ...
          <p className="text-slate-600 text-sm sm:text-base max-w-3xl leading-relaxed font-medium">
            هل تبحث عن قطعة غيار محددة؟ أو ترغب بالاستفسار عن أسعار القطع وبطاريات الهايبرد وكفالاتها؟ املأ النموذج بالأسفل أو تواصل معنا فوراً عبر الأرقام المتاحة.
          </p>
        </div>
      </section>
            ...
            <div className="lg:col-span-6 bg-white border border-slate-150 shadow-xl rounded-[2.5rem] p-8 sm:p-10 text-right order-1 lg:order-2">
              ...
              {isSubmitted ? (
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                  ...
                  >
                    <Mail size={16} />
                    <span>{isSubmitting ? "جاري الإرسال..." : "إرسال الاستفسار بالبريد الإلكتروني"}</span>
                  </button>
                </form>
              )}
            </div>
